...
public class PlayerMovement : MonoBehaviour
{
    ...
    public float fallMultiplier = 2.5f; // 떨어질 때 적용할 배수
    ...
    void Start()
    {
        ...
        if (!string.IsNullOrEmpty(ScenePortal.nextSpawnID))
        {
            ...
            foreach (SpawnPoint sp in spawnPoints)
            {
                ...
                {
                    // CharacterController가 켜져 있으면 위치 강제 이동이 무시될 수 있으므로 잠시 끕니다.
                    controller.enabled = false;
                    transform.position = sp.transform.position;
                    transform.rotation = sp.transform.rotation;
                    controller.enabled = true;
                    
                    Debug.Log($"SpawnPoint '{sp.spawnID}' 위치로 스폰되었습니다.");
                    break;
                }
            }
            // 사용한 ID는 초기화합니다.
    ...
    void Update()
    {
        ...
        // groundCheck가 할당되지 않았을 경우 경고를 띄우고 로직을 중단합니다.
        if (groundCheck == null)
        {
            Debug.LogWarning("Ground Check 오브젝트가 할당되지 않았습니다! 플레이어 발밑에 빈 오브젝트를 만들고 할당해주세요.");
            return;
        }

        // 발밑에 가상의 구체를 만들어 지면 레이어와 닿아있는지 체크
        ...
        if (isGrounded && velocity.y < 0)
            velocity.y = -2f; // 바닥에 닿아있을 때 아주 살짝 아래로 누르는 힘
        ...
        // 모든 이동(WASD + 중력/점프)을 하나의 벡터로 합쳐서 한 번만 실행
